...
export default function WaiterDashboard() {
  ...
  // Estado para guardar el bar_id del mesero actual
  ...
  const loadData = async () => {
    ...
    // 0. Obtener el perfil del mesero actual para extraer su bar_id
    ...
    // 1. Cargar productos filtrados estrictamente por el bar_id y activos
    ...
    // 2. Cargar órdenes activas filtradas por bar_id
    ...
    if (ordersData) {
      ...
      ordersData.forEach((ord: any) => {
        ...
        const formattedItems = ord.order_items?.map((item: any) => {
          const resolvedName = item.products?.name || productsMap.get(item.product_id) || 'Producto sin nombre';
          return {
            id: item.id,
  ...
  const handleSendOrder = async () => {
    ...
    // Si no existe orden activa, creamos una nueva pasando explícitamente el bar_id
    if (!targetOrderId) {
  ...
  if (loading) return <div className="p-8 text-white bg-slate-900 min-h-screen">Cargando sistema...</div>;

  const tablesList = ['Mesa 01', 'Mesa 02', 'Mesa 03', 'Mesa 04', 'Mesa 05', 'Mesa 06'];
    ...
    <main className="min-h-screen bg-slate-900 text-white p-4">
      <div className="max-w-md mx-auto">
        ...
        <header className="flex justify-between items-center mb-6 border-b border-slate-800 pb-3">
          <div>
            <span className="bg-amber-600 text-xs px-2.5 py-1 rounded-full font-semibold uppercase">Mesero</span>
            <h1 className="text-2xl font-bold mt-1">
              {selectedTable ? selectedTable : 'Mis Mesas'}
            </h1>
          </div>
              ...
              return (
                ...
                >
                  <div className="flex justify-between items-start">
                    ...
                    <span className={`w-2.5 h-2.5 rounded-full ${isBusy ? 'bg-amber-400 animate-pulse' : 'bg-emerald-400'}`}></span>
                  </div>
                  <div>
                    <p className={`text-xs font-semibold ${isBusy ? 'text-amber-400' : 'text-emerald-400'}`}>
                      {isBusy ? `Ocupada (${itemCount} items)` : 'Disponible'}
                    </p>
                  </div>
                </div>
              );
              ...
              <div className="space-y-4">
                <div className="bg-slate-800 border border-slate-700 p-4 rounded-xl space-y-3">
                  <div className="flex justify-between items-center border-b border-slate-700 pb-2">
                    <h2 className="text-sm font-bold uppercase tracking-wider text-slate-300">Cuenta Actual</h2>
                    <span className="text-xs bg-emerald-500/10 text-emerald-400 px-2 py-0.5 rounded font-semibold">
                      En consumo
                    </span>
                  </div>

                  {!activeTables[selectedTable] || activeTables[selectedTable].order_items.length === 0 ? (
                    ...
                    <div className="space-y-2 max-h-60 overflow-y-auto pr-1">
                      {activeTables[selectedTable].order_items.map((item, idx) => (
                        <div key={item.id || idx} className="bg-slate-900/70 p-2.5 rounded-lg flex justify-between items-start text-sm">
                          <div>
                            <p className="font-medium text-white">{item.product_name}</p>
                            {item.note && <p className="text-xs text-amber-400 mt-0.5">Nota: "{item.note}"</p>}
                          </div>
                  ...
                  {activeTables[selectedTable] && activeTables[selectedTable].order_items.length > 0 && (
                    <div className="border-t border-slate-700 pt-3 flex justify-between items-center">
                      <span className="text-sm text-slate-400 font-semibold">Total Acumulado:</span>
                      <span className="text-xl font-black text-emerald-400">
                        ${activeTables[selectedTable].order_items.reduce((acc, item) => acc + Number(item.price), 0).toFixed(2)}
                      </span>
                    </div>
                  )}
                </div>
              ...
              <div className="space-y-4">
                      ...
                      {currentOrder.map((item, idx) => (
                        <div key={idx} className="flex justify-between items-center bg-slate-900 p-2 rounded-lg text-xs">
                          <div>
                            ...
                            {item.note && <span className="text-amber-400 ml-1">({item.note})</span>}
                          </div>
                          <button 
                ...
                <div className="space-y-2">
                  {filteredProducts.map((product) => (
                    <div key={product.id} className="bg-slate-800 p-3 rounded-xl border border-slate-700 flex justify-between items-center">
                      <div>
                        <h4 className="font-medium text-white text-sm">{product.name}</h4>
                        <p className="text-xs text-slate-400">{product.category} • <span className="text-emerald-400 font-semibold">${product.price}</span></p>
                      </div>
                      <button 
        ...
        {activeProductForNote && (
          <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4 z-50">
            <div className="bg-slate-900 border border-slate-700 w-full max-w-sm p-6 rounded-2xl shadow-2xl space-y-4">
              <div>
                <span className="text-xs text-sky-400 font-semibold uppercase">Personalizar producto</span>
                <h3 className="text-lg font-bold text-white mt-0.5">{activeProductForNote.name}</h3>
              </div>
